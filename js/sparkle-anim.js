...
var sparklesLoadedArrayGlobal = new Set();

function stopAllSparkling()
{
	for(var i = 0; i < sparklesLoadedArrayGlobal.length; i++)
	{
		sparklesLoadedArrayGlobal[i].trigger("stop.sparkle");
	}
}

function loadSparklesForTab(tabChosen)
{
	if(tabChosen == "equipment")
	{
		var shinyItems = ["goldHelmet"];
		for(var i = 0; i < shinyItems.length; i++)
		{
			if(window[shinyItems] > 0)
			{
				if(sparklesLoadedArrayGlobal[i].contains())
				var shinyElement = $("#item-img-" + shinyItems[i]).sparkle();
				sparklesLoadedArrayGlobal.push(shinyElement);
			}
		}
	}
}
